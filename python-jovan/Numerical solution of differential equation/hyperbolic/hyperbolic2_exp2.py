...
def hyperbolic_operator_central(mesh, tau, a):
    ...
    A = diags([1 - abs(rx) - abs(ry)], [0], shape=(NN, NN), format='csr')
    val0 = np.broadcast_to(rx/2 + abs(rx)/2, (NN-n1, ))
    val1 = np.broadcast_to(ry/2 + abs(ry)/2, (NN-n0, ))
    I = k[1:, :].flat
    J = k[0:-1, :].flat
    A += csr_matrix((val0, (J, I)), shape=(NN, NN), dtype=mesh.ftype)
    A += csr_matrix((val1, (I, J)), shape=(NN, NN), dtype=mesh.ftype)

    val0 = np.broadcast_to(abs(rx)/2 - rx/2, (NN-n1, ))
    val1 = np.broadcast_to(abs(ry)/2 - ry/2, (NN-n0, ))                        
    I = k[1:, :].flat
    J = k[0:-1, :].flat
    ...
    return A
def hyperbolic_central(n, *fargs): # 点击这里查看 FEALPy 中的代码
    """
    @brief 时间步进格式为中心差分格式
    ...
    if n == 0:
        ...
        A = hyperbolic_operator_central(mesh,tau,a=1)
...
mesh.show_animation(fig, axes, box, hyperbolic_central, plot_type='surface', frames=nt+1)
plt.show()
